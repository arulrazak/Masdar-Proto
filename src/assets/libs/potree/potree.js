let measurement_type = null
let VOLUME_CAL = null
let VOLUME_STATUS = false
let pointcloudCom = []

let volumeValChangex = {
    aInternal: 1,
...
volumeValChangex.registerListener(async function(val) {

    // $('.potree_container').css('filter', 'blur(0px)')
    // $('.loading-screen-wrapper').addClass('show')
...
//volume calculation starts here
...
function calculatePolygonVolume({ positions: allPoints, area = defaultArea }) {
    // $('.potree_container').css('filter', 'blur(2px)')
...
let getAllPointsOfPointCloud = (pointCloud) => {
    var list = [];
...
Potree.Measure = class Measure extends THREE.Object3D {
    ...
    getArea() {
        ...
        // if (VOLUME_CAL) return VOLUME_CAL.toFixed(1) + ' ' + this.lengthUnit.code + '\u00B3'
        return Math.abs(area / 2).toFixed(1) + ' ' + this.lengthUnit.code + '\u00B2';
    };
    ...
    update() {
        ...
        { // update area label
            ...
            let msg;
            msg = Potree.utils.addCommas(this.getArea())
            this.areaLabel.setText(msg);
        }
    };

    updateText(volumeVal) {
        if (this.points.length === 0) {
            return;
        } else if (this.points.length === 1) {
            let point = this.points[0];
        ...
        { // update area label
            this.areaLabel.position.copy(centroid);
            this.areaLabel.visible = this.showArea && this.points.length >= 3;
            let msg = Potree.utils.addCommas(volumeVal.toFixed(1)) + ' ' + this.lengthUnit.code + '\u00B2';
            this.areaLabel.setText(msg);
        }
    }
...
Potree.MeasureVolumeHackV = class MeasureVolumeHackV extends THREE.Object3D {
    constructor() {
        super();
            ...
            let drop = e => {
                ...
                if (i !== -1) {
                    ...
                    if (VOLUME_STATUS) {
                        ...
                        let start = new Date()
                        this.volumeValue = calculatePolygonVolume({ positions: pointcloudCom, area: worldPoints })

                        // alert(val)
                    }

                    // console.log(calculatePolygonVolume({ positions: pointcloudCom, area: worldPoints }))
                    ...
                    //     worldPoints.push(point.position)
                    // })
                    // VOLUME_CAL = calculatePolygonVolume({ positions: getAllPointsOfPointCloud(this.viewer.scene.pointclouds[0]), area: worldPoints })
                    // this.volumeValue = VOLUME_CAL
                    // console.log("VOLUME_CAL", VOLUME_CAL);
                }
                // let points = this.points
                // let worldPoints = []
                // points.forEach(point => {
    ...
    getArea() {
        ...
        if (this.volumeValue) return this.volumeValue.toFixed(1) + ' ' + this.lengthUnit.code + '\u00B3'
        return 0
            // let area = 0;
            // let j = this.points.length - 1;
...
Potree.MeasuringTool = class MeasuringTool extends THREE.EventDispatcher {
    ...
    startInsertionVolume(args = {}) {

        let domElement = this.viewer.renderer.domElement;

        let measure = new Potree.MeasureVolumeHackV();
        ...
        let insertionCallback = async(e) => {
            ...
            if (e.button === THREE.MOUSE.LEFT) {
                ...
                // this.viewer.scene.measurements[this.viewer.scene.measurements.length - 1].areaLabel.setText("123")
                // this.update()
                ...
                console.log("onright click", this.viewer)
                    // calculatePolygonVolume(getAllPointsOfPointCloud(this.viewer.scene.pointclouds[0]), [])
                    // console.log("all pointcloud points", getAllPointsOfPointCloud(this.viewer.scene.pointclouds[0]))

                let points = this.viewer.scene.measurements[this.viewer.scene.measurements.length - 1].points
                ...
                pointcloudCom = getAllPointsOfPointCloud(this.viewer.scene.pointclouds[0])
                $('.loading-screen-wrapper').addClass('show')
                $.when(calculatePolygonVolume({ positions: pointcloudCom, area: worldPoints })).done(val => {
                    $('.loading-screen-wrapper').removeClass('show')

                    // VOLUME_CAL = calculatePolygonVolume({ positions: pointcloudCom, area: worldPoints })
                    // volumeValChangex.a = 1
                    VOLUME_STATUS = true
                        // VOLUME_CAL  = calculatePolygonVolume({})
                    console.log("volume--- ", val)

                    // $('body').removeAttr('style');
                    measure.volumeValue = val
                    console.log("val", val);
                    // alert(VOLUME_CAL)
                    // this.areaLabel.setText(msg);
                    // console.log("whole volume", volumeCal)
                    cancel.callback();
                })
        ...
        cancel.callback = e => {
            ...
            // measure.updateText(VOLUME_CAL)

            // this.viewer.scene.measurements[0].areaLabel.setText(VOLUME_CAL)
            // this.viewer.scene.measurements[0].areaLabel.update()
        };
    ...
    constructor(domElement, args = {}) {
        ...
        try {
            ...
            { // set defaults
                ...
                this.setNavigationMode(Potree.EarthControls);
...
initSidebar = (viewer) => {
    ...
    function initToolbar() {
        ...
        elToolbar.append(createToolIcon(
            ...
            function() {
                let volume = measuringTool.startInsertionVolume({
                    showDistances: false,
                    showArea: false,
                    closed: true,
                    name: 'Volume'
            ...
            clippingToolBar.append(createToolIcon(
                ...
                () => {
                    if (!(viewer.scene.getActiveCamera() instanceof THREE.OrthographicCamera)) {
                        viewer.postMessage(`Switch to Orthographic Camera Mode before using the Screen-Box-Select tool.`, { duration: 2000 });
                        return;
                    }
